class PurchasedProductsController < ApplicationController
  ...
  def index
    ...
    @purchased_products = @q.result(:distinct => true).includes(:coupons, :products, :customers).page(params[:page]).per(10)
  end
  ...
  private
    ...
    def purchased_product_params
      params.require(:purchased_product).permit(:products_id, :coupons_id, :customers_id, :quantity)
    end
end
